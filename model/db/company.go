...
func (c *Company) ParseImgList(ctx context.Context) []string {
	var imgList []string
	if err := sonic.UnmarshalString(c.CompanyImgList, &imgList); err != nil {
		klog.CtxErrorf(ctx, "[Company ParseImgList] UnmarshalString err: %v", err)
	}
	return imgList
}

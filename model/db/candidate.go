...
type Candidate struct {
	...
	Qualifications   string         `gorm:"column:qualifications;type:varchar(1024);comment:技能证书列表;not null;default:[];" json:"qualifications"`             // 技能证书列表
...
func AddCandidate(ctx context.Context, db *gorm.DB, candidate *Candidate) error {
...
func UpdateCandidate(ctx context.Context, db *gorm.DB, candidate *Candidate) error {
	if err := db.WithContext(ctx).Where("id = ?", candidate.Id).Updates(candidate).Error; err != nil {
		klog.CtxErrorf(ctx, "[DB] save candidate error: %v", err)
		return err
	}
	return nil

...
func BindTags(ctx context.Context, objId int64, tagIdList []int64) (int, error) {
	if err := authCreator(ctx, objId); err != nil {
		return 0, err
	}
	...
	tagRels, err := db.FindTagRelsByObjId(ctx, db.DB, objId, int32(sparkruntime.TagObjType_Recruitment))
	if err != nil {
		return 0, err
	}
	...
	tagObjRels := utils.MapStructList(modifyTagIdList, func(tagId int64) *db.TagObjRel {
		return &db.TagObjRel{
			...
			ObjId:   objId,
			TagId:   tagId,
			ObjType: int32(sparkruntime.TagObjType_Recruitment),
		}
	})
...
func authCreator(ctx context.Context, objId int64) error {
	...
	// find recruitment
	recruitment, err := db.FindRecruitmentById(ctx, db.DB, objId)
	if err != nil {
		return err
	}

	if recruitment.UserId != userId {
		return fmt.Errorf("recruitment creator id not match")
	}
